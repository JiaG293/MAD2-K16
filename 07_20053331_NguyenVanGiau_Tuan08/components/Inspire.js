...
export default function Inspire({ data }) {
    ...
    const renderItem = ({ item }) => (
        <Card style={{ margin: 10 }} contentStyle={styles.card}>
            <Card.Cover source={{ uri: item.image }} resizeMode='cover' style={{ width: "30%", height: 100}} />
            <Card.Content>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <Button mode="text" onPress={() => handleToggleCheck(item.id)}>
                        <MaterialCommunityIcons style={{ marginLeft: 50 }} name={check[item.id] ? "bookmark-check-outline" : "bookmark-outline"} color={"black"} size={26} />
                    </Button>
                </View>
    ...
    return (
        <View style={styles.container}>
            <View style={{
                ...
                <Text style={styles.textTitle}>Course that inspires</Text>
                <Button
                    mode="text"
